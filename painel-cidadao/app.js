...
(function () {
  ...
  // ============= TIMELINE DE SINAIS (relatorios.html) =============
  (function renderTimelineSinais() {
    ...
    // 2) Contratos acima de R$ 1 mi sem CNPJ valido
    contratos.forEach(c => {
      const valor = Number(c.valor) || 0;
    ...
    emendasAll.forEach(e => {
      ...
      if (valor >= 50000 && cruz.status === "sem_pagamento") {
        ...
        eventos.push({
          ...
          desc: `Emenda de ${esc(e.autor || "vereador nao identificado")} (Câmara) sem pagamento localizado pela Prefeitura.`,
  ...
  // ============= DETECTOR DE FRAGMENTAÇÃO (relatorios.html) =============
  (function detectarFragmentacao() {
    ...
    const contratosFrag = (pf.contratos || []).filter(c =>
      c.data_assinatura && c.contratado && (Number(c.valor) || 0) > 0 && (Number(c.valor) || 0) < LIMITE_DISPENSA
    );
  ...
  // ============= COMPARATIVO ENTRE ANOS POR CATEGORIA (relatorios.html) =============
  (function renderComparativoAnos() {
    ...
    contratosComp.forEach(c => {
      ...
      if (!ano) return;
      const cat = window.ZELA.classificarItem(c) || "outros";
      mapa[cat] = mapa[cat] || {};
...
<body>
  ...
  function initDiarias(prefix, dados) {
    ...
    const dimSecretaria = (d) => prefix === "Camara" ? (d.secretaria || d.unidade || "Camara Municipal") : (d.secretaria || d.unidade || "Não informado");
    const funcaoDiaria = (d) => {
      ...
      return {
        texto: "Funcao não informada na fonte",
    ...
    const secretarias = Array.from(new Set(lista.map(dimSecretaria).filter(Boolean))).sort((a, b) => a.localeCompare(b, "pt-BR"));
    ...
    const render = () => {
      ...
      if (statsEl) {
        ...
        statsEl.innerHTML = [
          ...
          { cls: "stat--gold", v: fmtNum(isPrefeitura ? view.length : qtd), l: isPrefeitura ? "Registros/empenhos" : "Quantidade de diárias", s: isPrefeitura ? "Consulta contabil da Prefeitura" : "Total informado na fonte" },
          { cls: "stat--teal", v: fmtBRL(isPrefeitura ? (total / (view.length || 1)) : media), l: isPrefeitura ? "Valor medio por registro" : "Valor medio por diária", s: isPrefeitura ? "Total dividido por registros" : "Total dividido pela quantidade" },
          { cls: "stat--navy", v: fmtNum(pessoas), l: "Funcionarios/vereadores", s: "Pessoas distintas no filtro" },
        ].map(s => `
          <div class="stat ${s.cls}">
            <div class="stat__value">${s.v}</div>
            <div class="stat__label">${s.l}</div>
      ...
      if (listaEl) {
        ...
        if (parent && !parent.querySelector(".diarias-note")) {
          ...
          note.className = "report-note diárias-note";
          note.innerHTML = prefix === "Prefeitura"
            ? `<strong>Diária não e salário extra:</strong> aqui a Prefeitura aparece como despesa/empenho classificado como diária. Pode ser legal, mas precisa ter processo, justificativa, liquidacao e comprovante. A fonte não garante que cada linha seja uma viagem individual.`
            : `<strong>Diária não e salário:</strong> na Camara, a diária deve indenizar viagem oficial. Para ser bem fiscalizada, precisa indicar destino, finalidade, autorizacao, período, prestacao de contas e resultado publico para Varginha.`;
      ...
      view.forEach(d => {
        ...
        if (!grupos[key].funcao || grupos[key].funcao === "Funcao não informada na fonte") {
        ...
        rankingEl.innerHTML = ranking.length ? `
          ...
          ${ranking.map((g, i) => `
            <div class="diaria-rank-row">
              <span class="diária-rank-row__pos">${i + 1}</span>
              <span class="diária-rank-row__person">
                <strong>${esc(cleanText(g.nome))}</strong>
                <small>Funcao: ${esc(cleanText(g.funcao))}</small>
              </span>
              <span>${esc(cleanText(g.secretaria))}</span>
              <span class="diária-rank-row__function">${esc(cleanText(g.funcaoDetalhe))}</span>
      ...
      listaEl.innerHTML = view.slice(0, 80).map(d => {
        const funcao = funcaoDiaria(d);
        return `
        <article class="diária-card">
          <div class="diária-card__value">
            <strong>${fmtBRL(d.valor_total || 0)}</strong>
            <span>${isPrefeituraCard ? "1 registro/empenho" : `${fmtNum(d.quantidade || 0)} diaria(s)`} · ${fmtBRL(d.valor_unitario || 0)} ${isPrefeituraCard ? "no registro" : "cada"}</span>
          </div>
          <div class="diária-card__body">
            <h4>${esc(cleanText(d.funcionario || "Funcionario nao informado"))}</h4>
            <div class="diária-card__role">
              <strong>Funcao/cargo:</strong> ${esc(cleanText(funcao.texto))}
              <small>${esc(cleanText(funcao.detalhe))}</small>
            </div>
            <p>${esc(cleanText(d.finalidade || d.historico || "Finalidade nao informada"))}</p>
            <div class="diária-card__meta">
              <span>${esc(cleanText(dimSecretaria(d)))}</span>
              ${d.destino ? `<span>Destino: ${esc(cleanText(d.destino))}</span>` : ""}
              ${d.numero ? `<span>No ${esc(d.numero)}</span>` : ""}
            ...
            <div style="margin-top:10px; display: flex; gap: 8px; flex-wrap: wrap;">
              ...
              <a class="btn-link" href="https://transparência.varginha.mg.gov.br/portal-transparência/consultas/diárias" target="_blank" title="Portal oficial da Prefeitura (pode estar temporariamente indisponível)" style="text-decoration:none; padding: 4px 8px; background: #eee; border-radius: 4px; color: #333; font-size: 0.75em; font-weight: 500; border: 1px solid #ccc; display: inline-block;">Fonte oficial</a>
              <a class="btn-link" href="https://transparência.betha.cloud/#/y7mn01LGqd_HCvGtj6VPwA==/diárias" target="_blank" title="Portal Betha — alternativa quando o portal oficial estiver fora do ar" style="text-decoration:none; padding: 4px 8px; background: #e8f4fd; border-radius: 4px; color: #1565c0; font-size: 0.75em; font-weight: 500; border: 1px solid #90caf9; display: inline-block;">Portal Betha</a>
            </div>
          </div>
        </article>`;
      }).join("");
    ...
    if (lista.length && contadorEl) {
      ...
      csvBtnD.addEventListener("click", () => {
        ...
        exportCSV(view.map(d => ({
          ...
          { key: "ano",           label: "Ano" },
        ], `diárias-${prefix.toLowerCase()}-varginha-${new Date().toISOString().slice(0,10)}.csv`);
      });
    }
  }
