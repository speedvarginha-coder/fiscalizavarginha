...
function mascararBlocosNaoVisiveis(html) {
  // Preserva as quebras de linha para que o número reportado continue correto.
  // A checagem anterior era feita linha a linha e, por isso, o regex nunca via
  // um bloco <script>...</script> completo: nomes de variáveis e chaves de API
  // acabavam tratados como texto mostrado ao cidadão.
  return html.replace(/<(script|style)\b[\s\S]*?<\/\1>/gi, (bloco) =>
    bloco.replace(/[^\r\n]/g, " ")
  );
}
...
for (const f of arquivos) {
  const html = mascararBlocosNaoVisiveis(readFileSync(join(ROOT, f), "utf8"));
